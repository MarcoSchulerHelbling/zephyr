...
int lsm6dsr_init_interrupt(const struct device *dev)
{
	...
	gpio_init_callback(&drv_data->gpio_cb,
			   lsm6dsr_gpio_callback, BIT(config->int_gpio.pin));
	...
	if (drv_data->hw_tf->update_reg(dev,
			       LSM6DSR_REG_INT1_CTRL,
			       LSM6DSR_MASK_INT1_CTRL_DRDY_XL |
			       LSM6DSR_MASK_INT1_CTRL_DRDY_G,
			       BIT(LSM6DSR_SHIFT_INT1_CTRL_DRDY_XL) |
			       BIT(LSM6DSR_SHIFT_INT1_CTRL_DRDY_G)) < 0) {
