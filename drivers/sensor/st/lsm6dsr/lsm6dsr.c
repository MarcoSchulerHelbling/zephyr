...
static int lsm6dsr_accel_range_set(const struct device *dev, int32_t range)
{
	...
	data->accel_sensitivity = (float)(lsm6dsr_accel_fs_sens[fs]
						    * SENSI_GRAIN_XL);
	return 0;
}
#endif
...
static int lsm6dsr_gyro_range_set(const struct device *dev, int32_t range)
{
	...
	data->gyro_sensitivity = (float)(lsm6dsr_gyro_fs_sens[fs]
						    * SENSI_GRAIN_G);
	return 0;
}
#endif
...
#if defined(CONFIG_LSM6DSR_EXT0_LIS2MDL) || defined(CONFIG_LSM6DSR_EXT0_LIS3MDL)
static int lsm6dsr_sample_fetch_magn(const struct device *dev)
{
	struct lsm6dsr_data *data = dev->data;
	uint8_t buf[6];

	if (lsm6dsr_shub_read_external_chip(dev, buf, sizeof(buf)) < 0) {
...
#endif
#if defined(CONFIG_LSM6DSR_EXT0_LPS22HB)
static int lsm6dsr_sample_fetch_press(const struct device *dev)
{
	struct lsm6dsr_data *data = dev->data;
	uint8_t buf[5];
				...
				enum sensor_channel chan)
{
	...
	case SENSOR_CHAN_MAGN_XYZ:
		lsm6dsr_sample_fetch_magn(dev);
		break;
#endif
#if defined(CONFIG_LSM6DSR_EXT0_LPS22HB)
	case SENSOR_CHAN_AMBIENT_TEMP:
	case SENSOR_CHAN_PRESS:
		lsm6dsr_sample_fetch_press(dev);
		break;
#endif
					    ...
					    float sensitivity)
{
	...
	case SENSOR_CHAN_ACCEL_XYZ:
		lsm6dsr_accel_convert(val, data->accel_sample_x, sensitivity);
		lsm6dsr_accel_convert(val + 1, data->accel_sample_y,
				      sensitivity);
		lsm6dsr_accel_convert(val + 2, data->accel_sample_z,
				      sensitivity);
		break;
	default:
		return -ENOTSUP;
...
static int lsm6dsr_accel_channel_get(enum sensor_channel chan,
				     ...
				     struct lsm6dsr_data *data)
{
	return lsm6dsr_accel_get_channel(chan, val, data,
...
static int lsm6dsr_gyro_channel_get(enum sensor_channel chan,
				    ...
				    struct lsm6dsr_data *data)
{
	return lsm6dsr_gyro_get_channel(chan, val, data,
					   ...
					   struct lsm6dsr_data *data)
{
	...
	case SENSOR_CHAN_MAGN_XYZ:
		lsm6dsr_magn_convert(val,
				     data->magn_sample_x,
				     data->magn_sensitivity);
		lsm6dsr_magn_convert(val + 1,
				     data->magn_sample_y,
				     data->magn_sensitivity);
		lsm6dsr_magn_convert(val + 2,
				     data->magn_sample_z,
				     data->magn_sensitivity);
		break;
	default:
...
static int lsm6dsr_magn_channel_get(enum sensor_channel chan,
				    struct sensor_value *val,
				    struct lsm6dsr_data *data)
{
	return lsm6dsr_magn_get_channel(chan, val, data);
}
#endif
			       ...
			       struct sensor_value *val)
{
	...
	case SENSOR_CHAN_MAGN_Z:
	case SENSOR_CHAN_MAGN_XYZ:
		lsm6dsr_magn_channel_get(chan, val, data);
		break;
#endif
#if defined(CONFIG_LSM6DSR_EXT0_LPS22HB)
	case SENSOR_CHAN_PRESS:
...
static int lsm6dsr_init_chip(const struct device *dev)
{
	...
	if (lsm6dsr_accel_set_fs_raw(dev,
				     LSM6DSR_DEFAULT_ACCEL_FULLSCALE) < 0) {
		LOG_DBG("failed to set accelerometer full-scale");
		return -EIO;
	}
...
static int lsm6dsr_init(const struct device *dev)
{
	...
	ret = lsm6dsr_shub_init_external_chip(dev);
	if (ret < 0) {
